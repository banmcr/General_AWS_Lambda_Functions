...
sns_arn = "arn:aws:sns:ap-south-1:11233445666:Cloudtrail_Create-Modify-delete"
...
def match_eventsource(txt) -> bool:
    # starts with
    print("event is = " + txt)
    expressions = (
...
def filter_user_events(event) -> bool:
    ...
    is_event_source = match_eventsource(event['eventSource'])
    is_in_event = 'invokedBy' in event['userIdentity'] and event['userIdentity']['invokedBy'] == 'AWS Internal'
    status = is_match and not is_read_only and not is_ignored_event and not is_event_source and not is_in_event
    print(event)
    print(event['eventSource'])
    print('useragent is ' + event['userAgent'])
    print('is_match is for useragent match ' + str(is_match))
    print('is_read_only is ' + str(is_read_only))
    print('is_ignored_event is ' + str(is_ignored_event))
    print('is_event_source is ' + str(is_event_source))
    print('is_in_event is ' + str(is_in_event))
